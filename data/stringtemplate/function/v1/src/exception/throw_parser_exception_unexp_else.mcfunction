...
data modify storage stringtemplate:string concatenate_sq.in set value ['Parsing error in file "', 'UNKNOWN', '": Unexpected $(else). $(else) should be placed between $(then) and $(endif). Like `$(if) NBT_PATH $(then) TEMPLATE [$(else) TEMPLATE] $(endif)`']
