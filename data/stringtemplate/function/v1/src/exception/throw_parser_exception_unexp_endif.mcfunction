...
data modify storage stringtemplate:string concatenate_sq.in set value ['Parsing error in file "', 'UNKNOWN', '": Unexpected $(endif). Expected `$(if) NBT_PATH $(then) TEMPLATE [$(else) TEMPLATE] $(endif)`']
