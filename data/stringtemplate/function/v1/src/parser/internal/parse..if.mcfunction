...
execute if data storage stringtemplate:parser self.tokens[0].IF \
    if data storage stringtemplate:parser self.tokens[1].TEXT \
    if data storage stringtemplate:parser self.tokens[2].THEN run \
...
execute if data storage stringtemplate:parser parse.local{pattern_valid:0b} run \
    return run function stringtemplate:v1/src/exception/throw_parser_exception_if
